...
| Axis dynamic zone | `TDynamicZoneAxisEmitterComponent`, same unit `:606` | A card-placement zone bounded by a straight line instead of a circle, e.g. "your whole half of the map". Bug to fix when ported (`docs/original-bugs.md`): `SetPosition` normalizes the point, so it only sits at distance 1 from the map centre unless fixed. | Yes |
...
| Link that buffs spawns | `TLinkEffectFireAtProducedUnitsComponent`, `...Server.Welas.pas:720` | While linked to a spawner, every unit it spawns gets an effect. Bug to fix when ported (`docs/original-bugs.md`): the hook keeps the link component's group, so only the spawner's groupless `eiWelaUnitProduced` reaches it. | Yes |
...
| Toon lighting | `TPostEffectToon.ToonType` `ttBoth` / `ttLighting`, `Engine/Engine.PostEffects.pas:403`, `PosteffectToon.fx` | Cel shading: the light buffer cut into bands (`LightThreshold`, `LightOffset`) plus a hard specular (`SpecularThreshold`), with or without the dark borders. `PostEffects.fxs` sets `ttBorder`: borders only. | No |
| Other FXAA presets | `TPostEffectFXAA.Mode` `fmLessDither` / `fmNoDither`, `Engine/Engine.PostEffects.pas:224` | Smoother, costlier edge search presets (20-29, 39). The stack uses `fmDither` quality 2 (preset 12). | Yes (the preset table) |
