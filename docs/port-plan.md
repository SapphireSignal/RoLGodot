...
| 4 | **Asset pipeline**: meshes (FBX + XML → scene/material), textures (+ KTF decoder), maps (terrain, water, vegetation, lights) | the Classic map and every unit render in a viewer scene, side-by-side screenshots checked. Meshes done (`docs/assets.md`: all 200 render and animate in the mesh viewer); open: the map (terrain, water, vegetation), glow / fur / outline / shadows |
