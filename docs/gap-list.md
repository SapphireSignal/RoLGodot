...
| Build grid behind each nexus: glowing tiles, a tile goes dark when its spawner fires (flash, 1 s fade), all glow in again after the rotation; colors while placing a card | `TBuildGridManagerComponent` | 🟨 tiles, glow and rotation ported (test_build_grid, captures; the original draws the same cyan glow, capture 2026-09-19); the activation particles and the placement colors' callers (input) wait for particles / phase 5 |
...
| Spawner buildings: the produced unit shown as a statue on its grid tile (spawners are invisible in the port, only the dark tile shows) | `TProductionPreviewComponent` (`SpawnerTemplate.dws:25`), `Modifiers\Spawner.dws` (drop-in animation, impact particles, glow) | ⬜ stub: next visible gap to port |
| Faded shapes over the water next to Classic's river rails (owner's screenshot 2026-09-19) | `Watershader.fx` refraction / reflection | ⬜ not the reflection (switching it off leaves them); refraction, or the pillars' underwater part seen through the water: compare against the original |
...
| Player HUD (seen in the original, capture mode): deck bar along the bottom (drop tiers left with the tier timer, spawner cards right with the nexus jump button), resource panel bottom left (gold with income, wood, charge bar; menu button), top panel (both nexus HP bars in %, match timer), minimap bottom right with the settings button | `HUD/DeckPanel`, `RessourcePanel`, `GameStatePanel`, `MinimapPanel`, `core_game*.scss` | ⬜ phase 7 (reference captures: `build/original/captures/`, `run_original.ps1 -Keys P`) |
| Sandbox HUD: dev panel top left (camera position, rotation / scroll / tilt / FoV offsets, lock / save / load camera, capture mode, clear units / spawners / towers, reset base buildings, overwatch), commander switch top right (one button per commander in the team color, click to control it), the deck raised and stacked by tier; P toggles capture mode (the player HUD, normal resources unless "Normal resources while capturing") | `HUD/Sandbox/Sandbox.dui`, `HUD/CommanderSwitch/CommanderSwitch.dui`, `core_game_deck.scss` `&.sandbox`, `Gamestates.pas:2704` | ⬜ phase 7 |
