...
func test_nexus_and_self() -> void:
	...
	var near_nexus := _unit(2, Vector2(50, 0))
	NexusMarker.new().Create(near_nexus)
	var far_nexus := _unit(3, Vector2(100, 0))
	NexusMarker.new().Create(far_nexus)
	check_eq(_update([RTarget.Create(own_nexus)], [4]), [far_nexus.ID], "the enemy nexus TryGetNexusNextEnemy gives (farthest)")
