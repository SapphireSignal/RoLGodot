...
const EFFECTS := ["MatcapShader.fx", "MetalShader.fx", "ColorOverlay.fx", "SpawnShader_White.fx",
	...
	"SpawnShader_Blue.fx"]
## Effects on one mesh at once, in order value order (spawn 1, tint 5000, matcap 9993, metal 10000).
	...
	for list: Array in lists:
		for flags: PackedStringArray in flag_sets:
			for cull in ["cmCCW", "cmNone"]:
				...
				for skinning in ([false, true] if flags.has("MATERIALTEXTURE") and flags.has("DIFFUSETEXTURE") else [false]):
					var shader := TMesh._shader_for(cull, flags, skinning, list)
					count += 1
					if RenderingServer.get_shader_parameter_list(shader.get_rid()).is_empty():
						failed.append("%s %s %s %s" % [list, flags, cull, skinning])
